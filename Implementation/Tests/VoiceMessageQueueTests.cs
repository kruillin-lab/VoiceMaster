...
namespace VoiceMaster.Tests;

/// <summary>
/// Unit tests for VoiceMessageQueue
/// Based on Echokraut's test patterns
/// </summary>
public class VoiceMessageQueueTests
{
    ...
    private static VoiceMessage MakeMessage(TextSource source = TextSource.AddonTalk, string text = "Hello.")
    {
        return new VoiceMessage
        {
            Text = text,
            OriginalText = text,
            Speaker = new NpcMapData(Dalamud.Game.ClientState.Objects.Enums.ObjectKind.EventNpc) { Name = "TestNpc" },
            Source = source,
        };
    }
