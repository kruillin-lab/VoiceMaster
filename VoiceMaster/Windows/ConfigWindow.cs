...
public class ConfigWindow : Window, IDisposable
{
    ...
    private List<VoiceMasterVoice> filteredVoices = [];
    ...
    public ConfigWindow() : base($"VoiceMaster Plugin.Configuration###EKSettings")
    {
    ...
    {
        var voiceArr = Plugin.Configuration!.VoiceMasterVoices.ConvertAll(p => p.ToString()).ToArray();
        var defaultVoiceIndex = Plugin.Configuration.VoiceMasterVoices.FindIndex(p => p.IsDefault);
        if (ImGui.Combo($"Default Voice:##EKDefaultVoice", ref defaultVoiceIndex, voiceArr, voiceArr.Length))
        {
            // Clear all defaults
            foreach (var voice in Plugin.Configuration.VoiceMasterVoices)
                voice.IsDefault = false;

            Plugin.Configuration.VoiceMasterVoices[defaultVoiceIndex].IsDefault = true;
            Plugin.Configuration.Save();
        }
        ...
        {
            filteredVoices = Plugin.Configuration.VoiceMasterVoices;
    ...
    private void DrawVoiceSelectionTable(string dataType, List<NpcMapData> realData, ref List<NpcMapData> filteredData, ref bool updateData, ref bool resetData, ref string filterGender, ref string filterRace, ref string filterName, ref string filterVoice, bool isBubble = false)
    {
        ...
        if (table)
        {
            ...
            foreach (NpcMapData mapData in filteredData)
            {
                ...
                {
                    var newVoiceItem = Plugin.Configuration!.VoiceMasterVoices.FindAll(f => f.IsSelectable(mapData.Name, mapData.Gender, mapData.Race, mapData.IsChild))[selectedIndexVoice];
    ...
    private async void BackendTestVoice(VoiceMasterVoice voice)
    {
        BackendStopVoice();
